...
def guardar_puntajes(usuarios):
    ...
    with open("puntajes.csv", "r") as usuarios:
        ...
        for i in range(len(lista_usuarios) - 1):
            ...
            jugadores["Nombre"] = lista_datos[0]
            jugadores["Puntaje"] = lista_datos[1]

    return jugadores

def mostrar_puntajes(lista_puntajes, x, y):
    '''
    Recibe la lista de los puntajes y los muestra por pantalla.
    '''
    fuente = pygame.font.Font(None, 24)
    mejores_puntajes = dict(sorted(lista_puntajes.items(), key = lambda item: item[1]))


    for clave, valor in mejores_puntajes.items():
        texto_clave = fuente.render(f"{clave}:", True, BLANCO)
        texto_valor = fuente.render(str(valor), True, BLANCO)
